...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        try
        {
            ...
            return options.Transport switch
            {
                ...
                _ => throw new NetcapException("Unsupported transport.")
            };
        }
        catch (NetcapException ex)
        {
            ...
            app.Use(async (ctx, next) =>
            {
                    ...
                    !System.Net.IPAddress.IsLoopback(remote))
                {
                    ...
                    await ctx.Response.WriteAsync(
                        "Netcap.Mcp HTTP transport accepts loopback only. " +
    ...
    {
        var help = """
            Netcap.Mcp - Model Context Protocol server for network trace capture.

            Usage:
              netcap-mcp [--stdio]                       Run with stdio transport (default)
              netcap-mcp --http [--port <n>] [--unsafe-listen-any]
                                                        Run with HTTP transport
              netcap-mcp --help                          Show this help
